...
TEST_P(SessionStateAddGetKernelTest, AddGetKernelTest) {
  auto tp = concurrency::CreateThreadPool(GetParam(), &onnxruntime::Env::Default(), ThreadOptions(),
                                          ORT_TSTR("SessionStateAddGetKernelTest"), true, nullptr);
...
TEST_P(SessionStateTestP, TestInitializerProcessing) {
  ...
  auto tp = concurrency::CreateThreadPool(param.thread_count, &onnxruntime::Env::Default(), ThreadOptions(),
                                          ORT_TSTR("SessionStateTest"), true, nullptr);
