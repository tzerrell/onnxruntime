...
typedef void(MLAS_POOL_KERNEL_ROUTINE)(const MLAS_WORK_BLOCK* WorkBlock, size_t ChannelCount, const float* Input,
                                       float* Output);
...
MLAS_DECLSPEC_ALIGN(static const float MlasInitialReductionInputIndex[], sizeof(MLAS_FLOAT32X4)) = {0.0f, 1.0f, 2.0f,
                                                                                                    3.0f};
...
struct MLAS_AVERAGE_POOLING {
  ...
  struct DividerVectorContext {
    ...
    MLAS_FLOAT32X4 DivideExcludePad(MLAS_FLOAT32X4 Reduction) {
      ...
      MLAS_FLOAT32X4 ReductionInputEndingIndex = MlasAddFloat32x4(ReductionInputIndex, KernelWidthBroadcast);

      MLAS_FLOAT32X4 LowerInputIndex = MlasMaximumFloat32x4(ReductionInputIndex, PaddingLowerBound);
      MLAS_FLOAT32X4 UpperInputIndex = MlasMinimumFloat32x4(ReductionInputEndingIndex, PaddingUpperBound);

      MLAS_FLOAT32X4 InputIndexDelta = MlasSubtractFloat32x4(UpperInputIndex, LowerInputIndex);

      //
      // Advance the input index vector for the next iteration.
      //

      ReductionInputIndex = MlasAddFloat32x4(ReductionInputIndex, MlasBroadcastFloat32x4(4.0f));
...
void MlasPool1DKernel(const MLAS_WORK_BLOCK* WorkBlock, size_t ChannelCount, const float* Input, float* Output)
/*++
...
template <typename PoolingType>
void MlasPool2DKernel(const MLAS_WORK_BLOCK* WorkBlock, size_t ChannelCount, const float* Input, float* Output)
/*++
...
template <typename PoolingType>
void MlasPool2DVectorKernel(const MLAS_WORK_BLOCK* WorkBlock, size_t ChannelCount, const float* Input, float* Output)
/*++
...
template <typename PoolingType>
void MlasPool3DKernel(const MLAS_WORK_BLOCK* WorkBlock, size_t ChannelCount, const float* Input, float* Output)
/*++
...
template <typename PoolingType>
void MlasPool3DVectorKernel(const MLAS_WORK_BLOCK* WorkBlock, size_t ChannelCount, const float* Input, float* Output)
/*++
...
template <typename PoolingType>
void MlasPoolGlobalKernel(const MLAS_WORK_BLOCK* WorkBlock, size_t ChannelCount, const float* Input, float* Output)
/*++
...
static void MlasPoolThreaded(void* Context, int32_t Index) {
  MLAS_WORK_BLOCK* WorkBlock = (MLAS_WORK_BLOCK*)Context;
  size_t TotalChannelCount = WorkBlock->TotalChannelCount;
  size_t TasksPerBlock = TotalChannelCount / WorkBlock->TargetThreadCount;
  size_t StartIndex = TasksPerBlock * Index;
  size_t EndIndex = StartIndex + TasksPerBlock;
  if (EndIndex > TotalChannelCount)
    EndIndex = TotalChannelCount;
...
void MLASCALL MlasPool(MLAS_POOLING_KIND PoolingKind, size_t Dimensions, const int64_t* InputShape,
                       const int64_t* KernelShape, const int64_t* Padding, const int64_t* StrideShape,
                       const int64_t* OutputShape, const float* Input, float* Output, MLAS_THREADPOOL* ThreadPool)
/*++
