...
class ThreadPool {
 public:
  ...
  int NumShardsUsedByTransformRangeConcurrently(int64_t block_size, int64_t total);
...
#ifdef USE_OPENMP
  template <typename F>
  inline static void TryParallelFor(concurrency::ThreadPool*, int32_t total, F&& fn) {
